...
(defun create-pipeline-layout (device layout-count set-layouts)
  (with-foreign-objects ((layout '%vk::pipeline-layout)
    ...
    (setf (mem-ref set-layout '%vk::descriptor-set-layout) set-layouts)
    (let ((*flags* nil)
	  (*set-layout-count* layout-count)
	  (*p-set-layouts* set-layout))
