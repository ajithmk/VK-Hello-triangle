...
(defun fill-mvp-buffer (device index m v p)
  (let ((size 192)
	(mvp-memory (nth index *mvp-memories*))
	(pointer nil))
    (with-foreign-object (mapped-pointer :pointer)
      (%vk::map-memory device mvp-memory 0 size 0 mapped-pointer)
      (setf pointer (mem-ref mapped-pointer :pointer))
      (iter (for loop-index from 0 below 16)
	    (setf (mem-aref pointer :float loop-index) (elt m loop-index)))
      (iter (for loop-index from 0 below 16)
	    (setf (mem-aref pointer :float (+ 16 loop-index)) (elt v loop-index)))
      (iter (for loop-index from 0 below 16)
	    (setf (mem-aref pointer :float (+ 32 loop-index)) (elt p loop-index)))
      (%vk::unmap-memory device mvp-memory))))
